...
class TestZone(unittest.TestCase):
    def test_bin_in_range(self):
        ...
        with self.assertWarns(UserWarning):
            bin_in_range(n_bin=0, axis=axis, limits=Limits(lower=-0.3, upper=-0.7))
        # Both limits inside bin
        self.assertTrue(
            bin_in_range(n_bin=0, axis=axis, limits=Limits(lower=-0.7, upper=-0.3))
        )
        # Lower limit outside bin
        self.assertTrue(
            bin_in_range(n_bin=0, axis=axis, limits=Limits(lower=-1.3, upper=-0.3))
        )
        ...
        self.assertTrue(
            bin_in_range(n_bin=0, axis=axis, limits=Limits(lower=-0.1, upper=-0.1))
        )
        # Equal limits on edges
        self.assertTrue(
            bin_in_range(n_bin=0, axis=axis, limits=Limits(lower=-1.0, upper=-1.0))
        )
        self.assertTrue(
            bin_in_range(n_bin=0, axis=axis, limits=Limits(lower=0.0, upper=0.0))
        )
        # Both limits outside bin
        self.assertTrue(
            bin_in_range(n_bin=0, axis=axis, limits=Limits(lower=-10.0, upper=10.0))
        )

        # Both limits below bin
        self.assertFalse(
            bin_in_range(n_bin=1, axis=axis, limits=Limits(lower=-1.0, upper=-0.5))
        )
        self.assertFalse(
        ...
        self.assertFalse(
            bin_in_range(n_bin=1, axis=axis, limits=Limits(lower=1.5, upper=2.5))
        )
        # Upper limit on edge
        self.assertTrue(
            bin_in_range(n_bin=1, axis=axis, limits=Limits(lower=-1.0, upper=0.0))
        )
        # Lower limit on edge
        self.assertTrue(
            bin_in_range(n_bin=1, axis=axis, limits=Limits(lower=1.0, upper=2.0))
        )
        # Both limits outside bin
        self.assertTrue(
