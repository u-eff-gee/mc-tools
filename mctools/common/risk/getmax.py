...
def createLaTeXCommands(configuration: Type[Configuration], path: Path = Path.cwd()):
    buffer = [
    ...
    scenarios_from_root_files = find_scenario_root_files(path=path)

    for scenario_name in scenarios_from_root_files:
        scenario = Scenario(name=scenario_name)
        scenario.addConfig(
            configuration(
                name="1A",
                rootfname=str(path / Path(scenario_name + ".root")),
                scalefname="scale.txt",
            )
        )
        ...
        for region in scenario.configurations[0].regions:
            ...
            for area in region.area:
                ...
                buffer.append("        " + ("}" * len(region.area[area].zones)) + "\n")
                buffer.append("      }{}%\n")
            buffer.append("    }{}%\n")
        buffer.append("  }{}%\n")
    buffer.append("}%\n")
